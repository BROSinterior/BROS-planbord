...
- Projectfiche → tabblad **Meetstaat**: "+ Lot toevoegen" zet een lot met zijn standaardposten klaar, "+ Post" kiest uit de bibliotheek of maakt een vrije post. Hoeveelheid, kostprijs, marge, locatie, btw en status vul je in de tabel in; totalen rekenen live. Volgorde wijzigen: sleep een post aan de greep ⋮⋮ vooraan de rij naar zijn plaats binnen het lot (op een touchscreen: pijltjes ▲▼); de nummers (lot.1, lot.2, …) en de groepskop volgen automatisch. Knop **Per ruimte**: de posten van elk lot alfabetisch gegroepeerd per ruimte (kolom Locatie), met aantal en subtotaal per ruimte — enkel een weergave voor intern gebruik (per browser onthouden); nummering, export en portaal veranderen niet, en slepen kan in die weergave niet.
