...
- Vereist `sql/016_werf.sql` (tabellen `werfbezoeken`, `vaststellingen`, bucket `werf`, realtime), `sql/018_werf_taken.sql`, `sql/019_vaststelling_titel.sql` en `sql/020_werf_plannen.sql`. Alleen het team (beheer + medewerker) kan lezen en schrijven; de klant ziet in stap 1 enkel de vaststellingen die hem als taak toegewezen zijn.
- Volgende stappen: (2b) werfverslag als pdf naar aannemers en klant (Drive-map Werfcontrole, mail), (3) aannemersportaal (eigen login, punten afwerken met foto), (4) checklists en oplevering.
